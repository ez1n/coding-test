...
function solution(park, routes) {
  let [x, y] = [0, 0];
  const map = { E: [0, 1], W: [0, -1], S: [1, 0], N: [-1, 0] };

  for (let i = 0; i < park.length; i++) {
    const sp = park[i].indexOf("S");
    if (sp > -1) {
      [x, y] = [i, sp];
      break;
    }
  ...
  routes.forEach((route) => {
    ...
    for (let i = 1; i <= +dist; i++) {
      const moveY = y + i * map[dir][1];
      const moveX = x + i * map[dir][0];

      if (!park[moveX] || !park[moveX][moveY] || park[moveX][moveY] === "X") {
        isAble = false;
        break;
      }
    }
