 ...
 * 이 휴대폰 자판은 키의 개수가 1개부터 최대 100개까지 있을 수 있으며, 특정 키를 눌렀을 때 입력되는 문자들도 무작위로 배열되어 있습니다. 또, 같은 문자가 자판 전체에 여러 번 할당된 경우도 있고, 키 하나에 같은 문자가 여러 번 할당된 경우도 있습니다.
...
function solution(keymap, targets) {
  ...
  keymap.forEach((key) => {
    Array.from(key).forEach((t, i) => {
      map.set(t, map.get(t) < i + 1 ? map.get(t) : i + 1);
    });
  });

  targets.forEach((target) => {
    const count = Array.from(target).reduce((sum, t) => (sum += map.get(t)), 0);
    answer.push(count || -1);
  });
